...
export default function AboutPage() {
  const { t } = useLanguage()
  const pillars = t.aboutPage.pillars.map((pillar, index) => ({
    icon: [MessageSquare, Brain, Building2, ShieldCheck][index] ?? MessageSquare,
    title: pillar.title,
          ...
          <div className="grid gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
            ...
            <div className="rounded-3xl border border-border bg-card p-6 shadow-lg">
              ...
              <div className="mt-5 rounded-2xl border border-leaf/30 bg-leaf/10 p-4 text-sm text-charcoal/75">
                ...
                <p className="mt-2 leading-relaxed">{t.aboutPage.communityText}</p>
              </div>
            </div>
          </div>
        </div>
